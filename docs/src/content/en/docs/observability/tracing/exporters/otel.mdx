...
For Laminar-specific features, consider using the dedicated [`@mastra/laminar`](/docs/v1/observability/tracing/exporters/laminar) exporter instead. It sets Laminar-native attributes (`lmnr.span.path`, `lmnr.span.ids_path`, `lmnr.association.properties.*`) for accurate span hierarchy rendering and tag visualization in the Laminar dashboard.
