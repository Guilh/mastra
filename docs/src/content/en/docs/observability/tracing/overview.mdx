...
You can import `DEFAULT_SERIALIZATION_OPTIONS` to reference or extend the default values:
  ...
  observability: new Observability({
    configs: {
      default: {
        serviceName: "my-service",
        serializationOptions: {
          // Double the default string length for larger payloads
          maxStringLength: DEFAULT_SERIALIZATION_OPTIONS.maxStringLength * 2,
          // Increase depth for deeply nested structures
          maxDepth: DEFAULT_SERIALIZATION_OPTIONS.maxDepth * 2,
        },
        exporters: [new DefaultExporter()],
      },
    },
