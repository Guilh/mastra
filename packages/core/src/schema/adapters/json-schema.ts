...
/**
 * Type guard to check if a value is a JsonSchemaWrapper.
 *
 * @param value - The value to check
 * @returns True if the value is a JsonSchemaWrapper
 *
 * @example
 * ```typescript
 * import { isJsonSchemaWrapper, toStandardSchema } from '@mastra/core/schema/adapters/json-schema';
 *
 * const schema = toStandardSchema({ type: 'string' });
 *
 * if (isJsonSchemaWrapper(schema)) {
 *   const originalSchema = schema.getSchema();
 * }
 * ```
 */
export function isJsonSchemaWrapper(value: unknown): value is JsonSchemaWrapper {
  return typeof value === 'object' && value !== null && value instanceof JsonSchemaWrapper;
}

/**
 * Type guard to check if a value is a JSON Schema wrapped with standard-schema.
 * This checks for the vendor being 'json-schema'.
 *
 * @param value - The value to check
 * @returns True if the value is a JSON Schema standard schema wrapper
 */
export function isJsonSchemaStandardSchema(value: unknown): value is StandardSchemaWithJSON {
  return (
    typeof value === 'object' &&
    value !== null &&
    '~standard' in value &&
    ...
    (value as any)['~standard'].vendor === VENDOR
  );
}
